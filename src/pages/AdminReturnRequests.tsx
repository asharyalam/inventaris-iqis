...
const AdminReturnRequests: React.FC = () => {
  ...
  const updateRequestStatusMutation = useMutation<void, Error, { requestId: string; status: 'Approved' | 'Rejected'; notes: string; item_id: string; quantity: number }>({
    ...
    onSuccess: () => {
      showSuccess("Permintaan pengembalian berhasil diperbarui!");
      queryClient.invalidateQueries({ queryKey: ['allReturnRequests'] });
      queryClient.invalidateQueries({ queryKey: ['userReturnRequests'] }); // Invalidate user's view
  ...
  const handleActionClick = (request: ReturnRequest) => {
  ...
  if (isLoading) {
    return <div className="text-center">Memuat daftar permintaan pengembalian...</div>;
  }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 space-y-6">
      <h2 className="text-3xl font-bold text-gray-900 mb-4">Manajemen Permintaan Pengembalian</h2>
      {requests && requests.length > 0 ? (
        ...
        <p className="text-center text-gray-500">Tidak ada permintaan pengembalian yang ditemukan.</p>
      )}
