...
const fetchApprovedConsumableRequests = async (): Promise<ConsumableRequest[]> => {
  const { data, error } = await supabase
    ...
    `)
    .eq('status', 'Approved by Headmaster')
...
const ConsumableRequestsAdminProcessingPage: React.FC = () => {
  ...
  const { data: requests, isLoading, error, refetch } = useQuery<ConsumableRequest[], Error>({
    queryKey: ['adminConsumableProcessing'],
    queryFn: fetchApprovedConsumableRequests,
  });

  const handleAction = async (status: 'Processed by Admin' | 'Rejected by Admin') => {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        approved_by: user.id, // Admin is now the one processing/approving
    ...
    if (updateError) {
      ...
      showSuccess(`Permintaan barang habis pakai berhasil ${status === 'Processed by Admin' ? 'diproses' : 'ditolak'} oleh Admin!`);
      // If processed, update item quantity
      if (status === 'Processed by Admin') {
      ...
      refetch();
      queryClient.invalidateQueries({ queryKey: ['consumableRequests'] }); // Invalidate user's list
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4">
      ...
      {requests && requests.length > 0 ? (
            ...
            {requests.map((request) => (
              <TableRow key={request.id}>
                ...
                <TableCell className="text-right">
                  {request.status === 'Approved by Headmaster' && (
        ...
        <p className="text-center text-gray-500">Tidak ada permintaan barang habis pakai yang perlu diproses.</p>
      )}

      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Proses Permintaan Barang Habis Pakai</DialogTitle>
          </DialogHeader>
          {selectedRequest && (
          ...
          <DialogFooter>
            <Button variant="destructive" onClick={() => handleAction('Rejected by Admin')}>Tolak</Button>
            <Button onClick={() => handleAction('Processed by Admin')}>Proses & Serahkan</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
export default ConsumableRequestsAdminProcessingPage;