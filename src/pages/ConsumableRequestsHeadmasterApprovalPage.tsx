...
const fetchPendingConsumableRequests = async (): Promise<ConsumableRequest[]> => {
...
const ConsumableRequestsHeadmasterApprovalPage: React.FC = () => {
  ...
  const { data: requests, isLoading, error, refetch } = useQuery<ConsumableRequest[], Error>({
    queryKey: ['headmasterConsumableApprovals'],
    queryFn: fetchPendingConsumableRequests,
  });
  ...
  const handleAction = async (status: 'Approved by Headmaster' | 'Rejected by Headmaster') => {
    ...
    if (updateError) {
      ...
      showSuccess(`Permintaan barang habis pakai berhasil ${status === 'Approved by Headmaster' ? 'disetujui' : 'ditolak'} oleh Kepala Sekolah!`);
      refetch();
      queryClient.invalidateQueries({ queryKey: ['adminConsumableProcessing'] }); // Invalidate admin processing page
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4">
      <h2 className="text-3xl font-bold mb-6 text-center">Persetujuan Permintaan Barang Habis Pakai</h2>
      {requests && requests.length > 0 ? (
        ...
        <p className="text-center text-gray-500">Tidak ada permintaan barang habis pakai yang perlu disetujui.</p>
      )}
...
export default ConsumableRequestsHeadmasterApprovalPage;