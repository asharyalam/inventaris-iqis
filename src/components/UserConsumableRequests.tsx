...
const fetchUserConsumableRequests = async (userId: string): Promise<ConsumableRequest[]> => {
...
const UserConsumableRequests: React.FC = () => {
  const { user, isLoading: isSessionLoading } = useSession();

  const { data: requests, isLoading, error } = useQuery<ConsumableRequest[], Error>({
    queryKey: ['userConsumableRequests', user?.id], // Unique query key
    queryFn: () => fetchUserConsumableRequests(user!.id),
    enabled: !!user && !isSessionLoading,
  });

  if (isSessionLoading || isLoading) {
    return <div className="text-center">Memuat riwayat permintaan barang habis pakai Anda...</div>;
  }

  if (error) {
    showError(`Gagal memuat riwayat permintaan barang habis pakai: ${error.message}`);
    return <div className="text-center text-red-500">Error: {error.message}</div>;
  }

  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <h3 className="text-2xl font-semibold mb-4">Riwayat Permintaan Barang Habis Pakai Anda</h3>
...
export default UserConsumableRequests;